<?php
// 1. HIDUPKAN ERROR REPORTING (Untuk nampak apa masalahnya)
...
if (!file_exists('vendor/autoload.php')) {
    die("Error: Fail 'vendor/autoload.php' tidak dijumpai. Sila pastikan anda telah run 'composer install'.");
}
...
// 4. CHECK ID
if (!isset($_GET['id'])) { 
    echo "<h3>Error: No ticket ID provided.</h3>"; 
    exit(); 
}
...
try {
    ...
    if (!$booking) { 
        echo "<h3>Error: Booking not found in database.</h3>"; 
        exit(); 
    }
    ...
    echo "Error Processing ID: " . $e->getMessage(); exit();
}
?>
...
<?php 
// Check jika footer wujud sebelum include untuk elak error
