...
<body>
    ...
    <script>
        let current = 0;
        let isMuted = true; // Default mesti Muted
        const items = document.querySelectorAll('.carousel-item');
        const vids = document.querySelectorAll('.video-bg');

        // Mainkan video pertama (Browser benarkan sebab muted)
        window.onload = () => { vids[0].play(); };

        function changeSlide() {
            let next = (current + 1) % items.length;
            
            // Set mute status video seterusnya ikut pilihan user
            ...
            // Delay sikit baru pause video lama (Supaya smooth transition)
            setTimeout(() => {
                vids[current].pause();
                current = next;
            }, 1200);
        ...
        function toggleMute() {
            ...
            // Ubah setting untuk video sekarang
            ...
            if(isMuted) {
                ...
                vids[current].play(); // Paksa play kalau tadi dia stop
            }
        }
